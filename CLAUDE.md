...
<!-- GSD:design-lock-start -->
## Design Lock — Frontend UI

**LOCKED. Do not deviate without explicit user approval.**

The authoritative frontend design is in `.planning/design_reference/design_handoff_f1_cockpit/`. All Phase 5+ frontend implementation must faithfully reproduce this design. The design files are:

- `README.md` — full spec: layout, tokens, interactions, data schema
- `design/cockpit-app.jsx` — App shell, TopStrip, Scrubber
- `design/cockpit-car.jsx` — Car panel (top-down chassis schematic)
- `design/cockpit-lap.jsx` — Lap Info panel (big time, deltas, sectors, pace, projection)
- `design/cockpit-map.jsx` — Track map panel
- `design/cockpit-physics.jsx` — Physics tabbed charts panel
- `design/data.jsx` — Data schema, mock fixture, color utilities
- `screenshots/` — Three reference captures

### Locked constraints (must not change)

**Layout:** Single screen, min 1600px. `grid-template-rows: 52px 1fr`. Main area: 3 columns (33% / 32% / 35%) × 2 rows. Car panel spans col 1 full height. Lap panel spans col 2 full height. Map = col 3 row 1; Physics = col 3 row 2. 1px gutters on `var(--rule)` background.

**Design tokens (exact values required):**
```css
--bg: #05070b; --panel: #0a0e15; --panel-bg: #070a11;
--panel-header: #0c1119; --panel-header-hi: #111827;
--rule: #1a2130; --rule-strong: #2a3445;
--text: #e8eef7; --text-dim: #6a7788; --text-muted: #46525f;
--accent: #00E5FF; --hot: #FF3344; --warn: #FFB020; --ok: #22E27A; --purple: #A855F7;
```

**Typography:** JetBrains Mono for all numerics, labels, headers. Inter as body fallback only. `font-feature-settings: "tnum" 1, "ss01" 1`. Big lap time = 56px weight 300. No rounded corners anywhere.

**Corner colors (Okabe-Ito):** FL `#E69F00`, FR `#56B4E9`, RL `#009E73`, RR `#F0E442`.

**Compound colors:** SOFT `#FF3333`, MEDIUM `#FFD700`, HARD `#FFFFFF`, INTER `#22C55E`, WET `#3B82F6`.

**Data schema (non-negotiable):** All per-corner metrics are CI triplets `{ mean, lo_95, hi_95 }`. Backend must return this shape. The physics panel renders CI bands from lo_95↔hi_95 and the mean line.

**State:** Single source of truth `pos` (float 1..MAX_LAP+0.999). `hoveredCorner` drives two-way sync between Car SVG wheels, footer readouts, and Physics charts — this is critical for legibility.

**Aesthetic rule:** Engineering-first (Mercedes/Red Bull pit-wall). No emoji. No gradients in the UI chrome. No rounded corners. Monospace numerics everywhere.
<!-- GSD:design-lock-end -->
